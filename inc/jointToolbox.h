...
//  jointToolbox.h
//  formerly rollingJoint.h
//  Main function is to allow user to create a "rolling joint" from a ball joint.  Other functions are provided as well
//  for example a function to extract the flex-extend, abd-add, int-ext rotational axes of a ball joint
...
//takes a reference frame from MuJoCo which is a rotation matrix and extracts the ith column to give a particular basis axis
void xmat2basis (mjtNum result[3], mjtNum xmat[9], int column)
{
  mjtNum xmatT[9];
  mju_transpose(xmatT, xmat, 3, 3);
  for (int i = 0; i < 3; i ++)
  {
    result[i] = xmatT[column * 3 + i];
  }
}
