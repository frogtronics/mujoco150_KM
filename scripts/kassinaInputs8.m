function kassinaInputs8(FILENAME, SAVEAS, trimStart, trimEnd)
...
if nargin < 2
    ...
    answer = inputdlg({'KM number','trial number'});
    savename = ['KM' answer{1} '_HOP_' answer{2}];
    SAVEAS = ['C:\Users\admin\Documents\Mujoco\mjpro150\input\kassina2016hops\' savename '.txt'];
end
...
if nargin<3
trim = inputdlg({'Trim start','Trim end'});
trimStart = str2double(trim(1));
trimEnd = str2double(trim(2));
end
...
% 
%     sacrum = data(:,(7*3-2):(7*3)); %THIS IS FOR 2016 DATA
%     snout = data(:,(6*3-2):(6*3));
% 
%     vecSpine = normalize(sacrum-snout);
%     
%     pSpine = sacrum./1000;
% 
%     %start at 0X, 0Y
...
[data_t,TM] = transformFrogData8(data);

snout = data_t(:,(6*3-2):(6*3));
sacrum = data_t(:,(7*3-2):(7*3));
...
%make the urostyle consistently angled down a bit (28.6 degrees)
vecUrostyle = repmat(normalize([0 -0.5 1]),length(sacrum),1);
...
%vecPelvis(:,1) = -vecPelvis(:,1); 
vecFemur(:,2) = -vecFemur(:,2);
 vecTibFib(:,2) = -vecTibFib(:,2);
 vecTarsal(:,2) = -vecTarsal(:,2);
...
for fr = 1:frames
 ...
 values = [values 0 0 0 1 0 0 0]; % add COP data for the newer version of Kassina model
...
if strfind(FILENAME,'RUN')%write the values to text
    for fr = 1:frames
        ...
        text = [text, sprintf('%2.6f,',phasedVals(fr,end))];
        text = [text, sprintf('%2.6f,',[0 0 0 1 0 0])];% add COP data for the newer version of Kassina model
        text = [text, sprintf('%2.6f\n', 0)];
    end
end
